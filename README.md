...
curl -fsSL https://raw.githubusercontent.com/arosyihuddin/cliproxyapiplus-installer/refs/heads/main/cliproxyapiplus-installer | bash

# Or clone and run manually
git clone https://github.com/arosyihuddin/cliproxyapiplus-installer.git
...
### 📋 Available Login and Import Flags (from ./cliproxyapi --help)
...
- `--github-copilot-login` - Login to GitHub Copilot using device flow
- `--gitlab-login` - Login to GitLab Duo using OAuth
- `--gitlab-token-login` - Login to GitLab Duo using a personal access token
- `--iflow-cookie` - Login to iFlow using Cookie
- `--iflow-login` - Login to iFlow using OAuth
- `--kilo-login` - Login to Kilo AI using device flow
- `--kimi-login` - Login to Kimi using OAuth
- `--kiro-aws-authcode` - Login to Kiro using AWS Builder ID (authorization code flow)
- `--kiro-aws-login` - Login to Kiro using AWS Builder ID (device code flow)
- `--kiro-google-login` - Login to Kiro using Google OAuth (same as --kiro-login)
- `--kiro-idc-login` - Login to Kiro using IAM Identity Center (IDC)
- `--kiro-import` - Import Kiro token from Kiro IDE
- `--kiro-login` - Login to Kiro using Google OAuth
- `--login` - Login Google Account
- `--qwen-login` - Login to Qwen using OAuth
- `--vertex-import` - Import Vertex service account key JSON file

### 🧰 Other Useful Flags

- `--config <path>` - Configure File Path
- `--incognito` - Open browser in incognito/private mode for OAuth
- `--no-browser` - Don't open browser automatically for OAuth
- `--no-incognito` - Force disable incognito mode (uses existing browser session)
- `--oauth-callback-port <port>` - Override OAuth callback port
- `--project_id <id>` - Project ID (Gemini only, not required)
